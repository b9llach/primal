...
IDirect3DDevice9 * C_CSGO::m_device() {
	if (!p_device)
		p_device = **reinterpret_cast<IDirect3DDevice9***>(util::FindSignature(crypt_str("shaderapidx9.dll"), csgo.signatures.at(0).c_str()) + 0x1);
...
IViewRenderBeams * C_CSGO::m_viewrenderbeams() {
	if (!p_viewrenderbeams)
		p_viewrenderbeams = *reinterpret_cast<IViewRenderBeams **>(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(1).c_str()) + 0x1);
...
CGlowObjectManager * C_CSGO::m_glow() {
	if (!p_glow)
		p_glow = *reinterpret_cast<CGlowObjectManager**>(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(2).c_str()) + 0x3);
...
IMoveHelper * C_CSGO::m_movehelper() {
	if (!p_movehelper)
		p_movehelper = **reinterpret_cast<IMoveHelper***>(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(3).c_str()) + 0x2);
...
CInput * C_CSGO::m_input() {
	if (!p_input)
		p_input = *reinterpret_cast<CInput**>(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(4).c_str()) + 0x1);
...
C_CSPlayerResource * C_CSGO::m_playerresource() {
	if (!p_playerresource)
		p_playerresource = **reinterpret_cast<C_CSPlayerResource ***>(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(5).c_str()) + 0x2);
	else if (csgo.globals.should_update_playerresource)
	{
		csgo.globals.should_update_playerresource = false;
		p_playerresource = **reinterpret_cast<C_CSPlayerResource ***>(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(5).c_str()) + 0x2);
	}
...
CSGameRulesProxy * C_CSGO::m_gamerules() {
	if (!p_gamerules)
		p_gamerules = **reinterpret_cast<CSGameRulesProxy ***>(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(6).c_str()) + 0x1);
	else if (csgo.globals.should_update_gamerules)
	{
		csgo.globals.should_update_gamerules = false;
		p_gamerules = **reinterpret_cast<CSGameRulesProxy ***>(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(6).c_str()) + 0x1);
	}
...
DWORD C_CSGO::m_postprocessing() {
	if (!p_postprocessing)
		p_postprocessing = *(DWORD*)(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(7).c_str()) + 0x2);
...
DWORD C_CSGO::m_ccsplayerrenderablevftable() {
	if (!p_ccsplayerrenderablevftable)
		p_ccsplayerrenderablevftable = *(DWORD *)(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(8).c_str()) + 0x4E);
...
IWeaponSystem* C_CSGO::m_weaponsys() {
	...
	if (!p_weaponsys)
		p_weaponsys = *reinterpret_cast<IWeaponSystem**>(util::FindSignature(crypt_str("client.dll"), csgo.signatures.at(21).c_str()) + 2);
