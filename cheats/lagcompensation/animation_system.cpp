...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	if (!player_info.fakeplayer && csgo.local()->is_alive() && e->m_iTeamNum() != csgo.local()->m_iTeamNum() && ticks_chocked >= 1)
	{
		...
		animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + 1 + (rand() % 58));
		...
		case RESOLVER_LOW_FIRST:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + (rand() % 35));
			break;
		case RESOLVER_LOW_SECOND:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y - (rand() % -35));
			break;
		case RESOLVER_LOW_FIRST1:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + (rand() % 20));
			break;
