...
void local_animations::update_local_animations(c_baseplayeranimationstate* animstate)
{
	...
	if (tickcount != m_globals()->m_tickcount) //-V550
	{
		...
		if (local_data.animstate)
		{
			flNewDuckAmount = math::clamp(csgo.local()->m_flDuckAmount() + animstate->m_fLandingDuckAdditiveSomething, 0.0f, 1.0f);
	...
	animstate->m_flGoalFeetYaw = antiaim::get().CanAntiAim(csgo.get_command(), false) ? local_animations::get().local_data.real_angles.y : abs_angles;

	player_t* local = csgo.local();

	local->force_bone_rebuild();
	memcpy(csgo.local()->get_animlayers(), layers, csgo.local()->animlayer_count() * sizeof(AnimationLayer));
	memcpy(&csgo.local()->m_flPoseParameter(), pose_parameter, 24 * sizeof(float));
	csgo.local()->set_abs_angles(Vector(0.0f, abs_angles, 0.0f));
	csgo.local()->set_abs_origin(csgo.local()->m_vecOrigin());
	csgo.local()->set_abs_velocity(csgo.local()->m_vecAbsVelocity());
	local->setup_local_bones(1);
