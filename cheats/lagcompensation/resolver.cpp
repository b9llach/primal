...
void resolver::resolve_yaw() {
    ...
    void ResolveAngles(entity_t * player); {
        ...
        if (!player->m_iShotsFired())
        {
            if (player)
            {
                switch (csgo.globals.missed_shots; 2)
                {
                case 0:
    ...
    float ResolveShotw(aim * data, lagcompensation * record);
    {
        ...
        float flPseudoFireYaw = math::normalize_yaw(csgo.local()->m_CachedBoneData()[0].GetOrigin().y);
...
#define MASK_SHOT_BRUSHONLY            (CONTENTS_SOLID|CONTENTS_MOVEABLE|CONTENTS_WINDOW|CONTENTS_DEBRIS)
    ...
    float  ResolverOvermake;
    {
        ...
        {
            int fired = csgo.globals.fired_shots[player->EntIndex()];
            int missed = csgo.globals.missed_shots[player->EntIndex()];
            auto& resolve_record = csgo.globals.revolver_working;
    ...
    void CResolver(); {
        ...
        float v6 = 0;
        for (size_t i = 0; i < player->animlayer_count(); i++) 
        {
        ...
        int v19 = csgo.globals.missed_shots[player->EntIndex()] % 2; 
        switch (v19)
        {
        case 0:
    ...
    switch (csgo.globals.missed_shots; 1)
    {
    case 1:
