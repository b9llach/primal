...
{
	if (/*fakelag::get().ispeeking() &&*/ vars.ragebot.lagpeek)
	{
		if (csgo.globals.shift_timer < 1)
		{
			csgo.globals.tickbase_shift = 0;
			++csgo.globals.shift_timer;
		}
		if (csgo.globals.shift_timer > 0)
		{
			csgo.globals.tickbase_shift = 16;
		}
	}
	else
	{
		csgo.globals.shift_timer = 0;
	}
}
