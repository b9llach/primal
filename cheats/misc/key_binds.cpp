...
void key_binds::update_key_bind(key_bind* key_bind, int key_bind_id)
{
	...
	case HOLD:
		...
		case 2:
			...
			if (misc::get().double_tap_key && vars.ragebot.double_tap_key.key != vars.antiaim.hide_shots_key.key)
		...
		case 12:
			...
			if (misc::get().hide_shots_key && vars.antiaim.hide_shots_key.key != vars.ragebot.double_tap_key.key)
...
{
	update_key_bind(&vars.legitbot.autofire_key, 0);
	update_key_bind(&vars.legitbot.key, 1);
	update_key_bind(&vars.ragebot.double_tap_key, 2);
	update_key_bind(&vars.ragebot.safe_point_key, 3);
	update_key_bind(&vars.ragebot.damage_override_key, 4);
	update_key_bind(&vars.antiaim.hide_shots_key, 12);
	update_key_bind(&vars.antiaim.manual_back, 13);
	update_key_bind(&vars.antiaim.manual_left, 14);
	update_key_bind(&vars.antiaim.manual_right, 15);
	update_key_bind(&vars.antiaim.flip_desync, 16);
	update_key_bind(&vars.misc.thirdperson_toggle, 17);
	update_key_bind(&vars.misc.automatic_peek, 18);
	update_key_bind(&vars.misc.edge_jump, 19);
	update_key_bind(&vars.misc.fakeduck_key, 20);
	update_key_bind(&vars.misc.slowwalk_key, 21);
	update_key_bind(&vars.ragebot.body_aim_key, 22);
	update_key_bind(&vars.ragebot.dt_key, 23);
	update_key_bind(&vars.antiaim.Auto_FreeStand_key, 24);
}
...
bool key_binds::get_key_bind_state_lua(int key_bind_id)
{
	...
	case 4:
		if (csgo.globals.current_weapon < 0)
			return false;

		return keys[4 + csgo.globals.current_weapon];
	case 12:
