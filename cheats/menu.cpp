...
//			file_path = std::string(path) + crypt_str("\\primal\\lua\\");
//
...
//			add_key("Auto Freestand", vars.antiaim.Auto_FreeStand && antiaim::get().Auto_FreeStand_key || key_binds::get().get_key_bind_state(_FREESTAND), vars.antiaim.Auto_FreeStand_key, 200, min(main_alpha * 2, 1.f), rage, false);
...
//
//			add_key("Auto-Freestand", key_binds::get().get_key_bind_state(_AUTO_PEEK), vars.antiaim.Auto_FreeStand_key, 200, min(main_alpha * 2, 1.f), true);
//		}
//		ImGui::EndGroup();
//		ImGui::PopFont();
//	}
...
//}
//
//#define VERSION crypt_str("primal")
...
//			ImGui::PushFont(font);
//			draw->AddText(ImVec2(p.x + 5, p.y + s.y - 20 + (10 - ImGui::CalcTextSize(std::string("primal " + build + " | build at: " + build_at + " | " + sponsor).c_str()).y / 2)), ImColor(220, 220, 220, int(public_alpha * 255)), std::string("primal " + build + " | build at: " + build_at + " | " + sponsor).c_str());
...
//					ss -= 0.04f;
//				ImGui::PushFont(name);
//				ImGui::GetForegroundDrawList()->AddText(ImVec2(p.x + tab_anim / 2 - ImGui::CalcTextSize("primal").x / 2, p.y + 20 - ImGui::CalcTextSize("primal").y / 2), ImColor(vars.misc.menu_color.r() / 255.f, vars.misc.menu_color.g() / 255.f, vars.misc.menu_color.b() / 255.f, ImClamp(ss * 2.f, 0.f, 1.f)), "primal");
//				ImGui::PopFont();
//			}
//		}
//		
...
//
//								ImGui::Checkbox(crypt_str("New Exploit"), &vars.antiaim.negromode);
//
...
//							if (!vars.antiaim.antiaim_type) {
//
//								ImGui::Checkbox(crypt_str("Freestand"), &vars.antiaim.Auto_FreeStand);
//								//ImGui::SameLine();
//								//draw_keybind(crypt_str(""), &vars.antiaim.Auto_FreeStand_key, crypt_str("##HOTKEY_AUTOFREESTAND"));
//
//								//draw_keybind(crypt_str("Auto peek"), &vars.antiaim.Auto_FreeStand_key, crypt_str("##AUTOFREESTAND__HOTKEY"), true);
//
//
//
...
//
//								if (vars.esp.removals[REMOVALS_ZOOM])
//									ImGui::Checkbox(crypt_str("Fix zoom sensivity"), &vars.esp.fix_zoom_sensivity);
//
//								if (vars.esp.removals[REMOVALS_ZOOM])
//									ImGui::SliderInt(crypt_str("Scope Zoom"), &vars.esp.Zoom, 0, 150);
//
...
void load_config()
{
	...
	eventlogs::get().add(crypt_str("Loaded ") + files.at(vars.selected_config) + crypt_str(" config"), false);
}
					...
					ImGui::BeginGroup(/*CHILDs SPACE START*/);
					{
						ImGui::MenuChild("Configs", ImVec2(290, 420));
						{
							...
							ImGui::BeginChild("##configs", ImVec2(290, 300), false, ImGuiWindowFlags_NoBackground);
							{
								...
								//draw_config_button("Add new config", "adddadasdasdsadsadassaasd1", true, false, false, false);
								//for (int i = 0; i < files.size(); i++)
								//{
								//	bool load = true, save = false;
								//	draw_config_button(files.at(i).c_str(), std::string(files.at(i) + "id_ajdajdadmadasd1").c_str(), load, save, i);
								//}

								//draw_config_button("Add new config", "adddadasdasdsadsadassaasd2", false, true, false, false);
								//for (int i = 0; i < files.size(); i++)
								//{
								//	bool load = false, save = true;
								//	draw_config_button(files.at(i).c_str(), std::string(files.at(i) + "id_ajdajdadmadasd2").c_str(), load, save, i);
								//}
							}
							ImGui::EndChild();
						}
