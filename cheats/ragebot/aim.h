...
public:
	...
	scanned_target(adjust_data* record, const scan_data& data) //-V688 //-V818
	{
		...
		auto aim_angle = math::calculate_angle(csgo.globals.eye_pos, data.point.point); //-V688
		auto fov = math::get_fov(viewangles, aim_angle); //-V688

		this->fov = fov;
		this->distance = csgo.globals.eye_pos.DistTo(data.point.point);
...
public:
	...
	void threaded_run(CUserCmd* cmd);
