...
bool antiaim::CanAntiAim(CUserCmd* m_pcmd, bool extra_check) {
	...
	if (!weapon
		...
		|| !csgo.available()
		|| !csgo.local()->is_alive()
		|| csgo.local()->m_bGunGameImmunity()
		|| csgo.local()->m_fFlags() & FL_FROZEN
		|| csgo.local()->get_move_type() == MOVETYPE_NOCLIP
		|| csgo.local()->get_move_type() == MOVETYPE_LADDER
		|| (m_pcmd->m_buttons & IN_ATTACK && weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && !weapon->is_non_aim())
		|| (weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER && !csgo.globals.revolver_working && (m_pcmd->m_buttons & IN_ATTACK || m_pcmd->m_buttons & IN_ATTACK2))
...
float antiaim::The_silence_of_the_Buryats(CUserCmd* m_pcmd)
{
	static int lastBuryat = 0;
	static int buryatSide = -1;
	int currentBuryat = lastBuryat;
	...
	if (csgo.send_packet)
	{
		...
		if (m_pcmd->m_command_number % 3 == 0)
		{

			csgo.globals.break_lc = true;
			csgo.globals.tickbase_shift = 3;
		}
		else
		{
			buryatSide *= -1; //возвращаем сайд на его блядское место
		}


	}

	lastBuryat = currentBuryat;
	return currentBuryat * buryatSide;
}

float antiaim::UpPitchExploit(CUserCmd* m_pcmd)
{
	...
	{
		pitch = math::random_float(-89.0f, 89.0f); //invert_jitter ? (vars.misc.anti_untrusted ? 89.0f : 540.0f) : (vars.misc.anti_untrusted ? -89.0f : -540.0f);
	}
	else
	{
		pitch = get_pitch(m_pcmd);
	}
...
float antiaim::randomyaw(CUserCmd* m_pcmd)
{
	auto yaw = m_pcmd->m_viewangles.y;

	yaw = math::random_float(-58, 58);
...
void antiaim::create_move(CUserCmd* m_pcmd)
{
	...
	if (!vars.antiaim.negromode)
		m_pcmd->m_viewangles.y = get_yaw(m_pcmd);
	else if (vars.antiaim.RandomYaw)
	{
		m_pcmd->m_viewangles.y = randomyaw(m_pcmd);
	}
	else
	{
		m_pcmd->m_viewangles.y = at_targets();
		m_pcmd->m_viewangles.y += The_silence_of_the_Buryats(m_pcmd);
	}
	if (!vars.antiaim.UpPitchEx)
		m_pcmd->m_viewangles.x = get_pitch(m_pcmd);
	else
	{
		m_pcmd->m_viewangles.x += UpPitchExploit(m_pcmd);
	}
}
...
{
	if (vars.antiaim.Auto_FreeStand)
	{
		float Right, Left;
		Vector src3D, dst3D, forward, right, up;
		trace_t tr;
