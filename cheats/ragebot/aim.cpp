...
bool aim::SanityCheck(CUserCmd* cmd, bool weapon, int idx, bool check_weapon) {
	...
	bool has_revolver = (csgo.globals.weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER);
	...
	if (!weapon) {
		...
		if (check_weapon && !csgo.globals.weapon->can_fire(has_revolver))
			return false;
	...
	{
		if (csgo.globals.weapon->is_non_aim() || idx == -1)
			return false;
...
void aim::run(CUserCmd* cmd)
{
	...
	int idx = csgo.globals.current_weapon;
	...
	if (final_target.data.valid())
	{
		...
		auto unlag = fabs(csgo.local()->m_flSimulationTime() - csgo.local()->m_flOldSimulationTime()) < interval_per_tick;
...
void aim::fire(CUserCmd* cmd)
{
	...
	shot->shot_info.final_backtrack_ticks = backtrack_ticks;
