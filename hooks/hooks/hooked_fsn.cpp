...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	if (stage == FRAME_RENDER_END)
	{
		...
		if (vars.player.enable && vars.esp.removals[REMOVALS_ZOOM] && vars.esp.fix_zoom_sensivity)
			zoom_sensitivity_ratio_mouse->SetValue(0.0f);
		else
			zoom_sensitivity_ratio_mouse->SetValue(1.0f);
			...
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					log << "Fired shot to " << current_shot->shot_info.target_name << "'s "
						...
						<< reason << " backtracked for " << current_shot->shot_info.final_backtrack_ticks << " hitchance " << current_shot->shot_info.hitchance
						<< " [not successful]";
				...
				else
				{
					...
					log << "Fired shot to " << current_shot->shot_info.target_name << "'s "
						...
						<< current_shot->shot_info.server_hitbox << " for " << current_shot->shot_info.server_damage << " backtracked for " << current_shot->shot_info.final_backtrack_ticks
