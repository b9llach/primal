...
namespace ns_globals
{
	int get_framerate()
	{
		return csgo.globals.framerate;
	}

	int get_ping()
	{
		return csgo.globals.ping;
	}
	...
	std::string get_time()
	{
		return csgo.globals.time;
	}

	std::string get_username()
	{
		return csgo.username;
	}
...
namespace ns_events
{
	...
	{
		if (std::find(csgo.globals.events.begin(), csgo.globals.events.end(), event_name) == csgo.globals.events.end())
		{
			m_eventmanager()->AddListener(&hooks::hooked_events, event_name.c_str(), false);
			csgo.globals.events.emplace_back(event_name);
		}
...
namespace ns_cmd
{
	...
	bool get_button_state(int button)
	{
		...
		return csgo.get_command()->m_buttons & button;
	}
	...
	void set_button_state(int button, bool state)
	{
		...
		if (state)
			csgo.get_command()->m_buttons |= button;
		else
			csgo.get_command()->m_buttons &= ~button;
	}
}
...
void c_lua::refresh_scripts()
{
	...
	{
		folder = std::string(path) + crypt_str("\\primal\\lua\\");
...
void c_lua::load_script(int id)
{
	...
	if (error_load | loaded.at(id) == false)
	{
	 ...
	 //-V106
	csgo.globals.loaded_script = true;
}
